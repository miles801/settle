...
public class SecurityClient {
    ...
    /**
     * @param defaultTaobaoClient serverUrl必须是https协议
     * @param randomNum           伪随机码
     */
     ...
     * 解密（每个用户单独分配秘钥）
     *
     * @param data    密文数据 手机号码格式：$手机号码前3位明文$base64(encrypt(phone后8位置))$111$
     *                nick格式：~base64(encrypt(nick))~111~
     * @param type    解密字段类型(例如：nick\phone)
     * @param session 用户身份,用户级加密必填
     * @return
     */
     ...
     *
     * @param dataList
     * @param type     加密字段类型(例如：nick\phone)
     * @return
     * @throws SecretException
