...
public class HibernateInterceptor extends EmptyInterceptor {
    ...
    public boolean onSave(Object entity, Serializable id, Object[] state, String[] propertyNames, Type[] types) {
        ...
        if (entity instanceof CommonDomain) {
            ...
            if (userId != -1 && !StringUtils.isEmpty(SecurityContext.getEmpId())) {
                state[userId] = SecurityContext.getEmpId();
                o.setCreatorId(SecurityContext.getEmpId());
            }
            ...
            if (StringUtils.isEmpty(o.getOrgId())) {
                int orgId = ArrayUtils.indexOf(propertyNames, SystemPropertyConstant.ORG_ID);
                if (orgId != -1 && StringUtils.isNotEmpty(SecurityContext.getOrgId())) {
                    state[orgId] = SecurityContext.getOrgId();
                    o.setOrgId(SecurityContext.getOrgId());
                }
            }
            return true;
        }
